...
optimizerG = torch.optim.Adam([{"params":G1.parameters()},{"params":G2.parameters()},{"params":G3.parameters()}], lr=param.lr_G, betas=(param.beta1, param.beta2), weight_decay=param.weight_decay)
...
if param.load_ckpt:
    ...
    G2.load_state_dict(checkpoint['G3_state'])
...
for i in range(iter_offset, param.total_iters):
    ...
    lossD, lossD_real, lossD_fake1, lossD_fake2, lossD_fake3, loss1 = learnD_Realness(param, D, G1, G2, G3, optimizerD, random_sample, Triplet_Loss, x, anchor1, anchor0)
    lossG, lossG1, lossG2, lossG3, loss_MSE = learnG_Realness(param, D, G1, G2, G3, optimizerG, random_sample, Triplet_Loss, x, anchor1, anchor0)
    ...
    if i < 1000 or (i+1) % 100 == 0:
        ...
        #             end - start)
        fmt = '[%d / %d   ] SD: %d Diff: %.4f loss_D: %.4f loss_real: %.4f lossD_fake1: %.4f lossD_fake2: %.4f lossD_fake3: %.4f loss1:  %.4f loss_G:  %.4f loss_G1: %.4f loss_G2: %.4f loss_G3: %.4f loss_MSE: %.4f time:%.2f'
        s = fmt % (i + 1, param.total_iters, param.seed,
                   -lossD.data.item() + lossG.data.item() if (lossD is not None) and (lossG is not None) else -1.0,
                   lossD.data.item() if lossD is not None else -1.0,
                   ...
                   lossD_fake3.data.item() if lossD_fake3 is not None else -1.0,
                   ...
                   lossG3.data.item() if lossG3 is not None else -1.0,
    ...
    if (i+1) % param.gen_every == 0:
        ...
        with torch.no_grad():
            ...
            for ext in range(int(param.gen_extra_images/extra_batch)):
                ...
                fake_test3 = G3(z_extra.normal_(0, 1)).squeeze()
                fake_test3 = fake_test3.cpu().clone().numpy()
                fake_test_list3.extend(fake_test3)
            ...
            with open('%s/%01d/extra3.pk' % (param.extra_folder, current_set_images), 'wb') as f:
                pickle.dump(fake_test_list3, f, protocol=pickle.HIGHEST_PROTOCOL)
