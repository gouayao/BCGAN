...
def learnG_Realness(param, D, G1, G2, G3, optimizerG, random_sample, Triplet_Loss, x, anchor1, anchor0):
    ...
    for t in range(param.G_updates):
        ...
        for _ in range(param.effective_batch_size // param.batch_size):
            ...
            imgs_fake3 = G3(z)
            feat_fake3 = D(imgs_fake3).log_softmax(1).exp()
            ...
            if param.relativisticG:
                ...
                #lossG2.backward()
                lossG3 = -Triplet_Loss(anchor_fake, feat_fake3, skewness=param.negative_skew) + Triplet_Loss(feat_real, feat_fake3)
                # lossG2.backward()
                loss_mse = nn.MSELoss()
                loss_MSE = loss_mse(imgs_fake1, imgs_fake2) +loss_mse(imgs_fake1, imgs_fake3) + loss_mse(imgs_fake2, imgs_fake3)
                lossG = lossG1 + lossG2 + lossG3 - 0.005 * loss_MSE
    ...
    return lossG, lossG1, lossG2, lossG2, loss_MSE
